...
const Sidebar = ({
  ...
  //         <SidebarLinkGroup
  //           activeCondition={
  //             pathname === '/prepress' || pathname.includes('prepress')
  //           }
  //         >
  //           {(handleClick, open) => {
  //             return (
  ...
  //       <>
  //         <SidebarLinkGroup
  //           activeCondition={pathname === '/ppic' || pathname.includes('ppic')}
  //         >
  //           {(handleClick, open) => {
  //             return (
  //               <React.Fragment>
  ...
  //                   <SidebarLinkGroup
  //                     activeCondition={
  //                       pathname === '/ppic/master' ||
  //                       pathname.includes('ppic/master')
  //                     }
  //                   >
  //                     {(handleClick, open) => {
  //                       return (
  ...
  async function getmasterAkses() {
    const url = `${import.meta.env.VITE_API_LINK}/master/role/1`;
    try {
      const res = await axios.get(url, {
        withCredentials: true,
      });
      setMapData(res.data.data.akses);
  ...
  const renderTest = () => {
    return (
      <>
        {mapData.map((data: any, index: number) => {
          return (
            data.is_main == false?
            <li>
             <NavLink
            ...
            <SidebarLinkGroup
              activeCondition={
                pathname === data.path_name || pathname.includes(data.path_name)
              }
            >
              {(handleClick, open) => {
                return (
                  <React.Fragment>
                    {data.parent_1.map((parent1: any, iParent1: number) => {
                      return (
                        <>
                          <NavLink
                            to={parent1.path_name}
                            className={({ isActive }) =>
                              `group relative flex items-center mb-4 gap-5 rounded-sm py-2 px-4 font-medium !text-white duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4` +
                              (isActive &&
                                '!text-[#0065DE] bg-white text-primary py-3 px-1 text-[16px]')
                          ...
                          {parent1.parent_2.map((parent2: any, iParent2: number) => {
                            return (
                              parent2.is_main == true ?
                                <div
                                  className={`translate pl-3 transform overflow-hidden ${!open && 'hidden'
                                    }`}
                                >
                                  <li>
                                    <SidebarLinkGroup
                                      activeCondition={
                                        pathname === parent2.path_name || pathname.includes(parent2.path_name)
                                      }
                                    >
                                      {(handleClick, open) => {
                                        return (
                                          <React.Fragment>
                                            {
                                              parent2.parent_3.map((parent3: any, iParent3: number) => {
                                                return (
                                                  <>
                                                    <NavLink
                                                      to="#"
                                                      className={`group relative flex items-center gap-5 mb-2 rounded-sm py-2 px-4 font-medium text-bodydark1 duration-300 ease-in-out  ${(pathname === parent3.path_name ||
                                                          pathname.includes(parent3.path_name)) &&
                                                        ' dark:bg-meta-4'
                                                        }`}
                                                      onClick={(e) => {
                                                        e.preventDefault();
                                                    ...
                                                    {/* <!-- Dropdown Menu Start --> */}
                                                    {
                                                      parent3.parent_4.map((parent4: any, iParent4: number) => {
                                                        return (
                                                          <div
                                                            className={`translate transform overflow-hidden ${!open && 'hidden'
                                                              }`}
        ...
        <nav className="mt-5 py-4 px-4 lg:mt-9 lg:px-6">
          ...
          <div>
            <ul className="mb-6 flex flex-col gap-1.5">
              ...
              {role === 'admin' && bagian === 'hr' && renderHRs()}

              {role === 'supervisor' && bagian === 'hr' && renderHRs()}

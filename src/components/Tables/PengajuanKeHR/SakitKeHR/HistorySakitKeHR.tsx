...
function HistorySakitKeHR() {
  ...
  async function getMe() {
    ...
    try {
      ...
      getSakit(res?.data.karyawan.biodata_karyawan[0]?.id_department);
  ...
  async function getSakit(idDept: any) {
  ...
  const handleResetFilters = () => {
    ...
    setTimeout(() => getSakit(idPengaju), 100);
  };

  const exportToExcel = () => {
    if (!sakit?.data || sakit.data.length === 0) {
      alert('No data to export');
      return;
    }

    // Prepare data for Excel
    const excelData = sakit.data.map((item: any, index: number) => ({
      No: index + 1,
      ...
      'Tanggal Mulai Sakit': dateOnly(item.dari),
      'Tanggal Selesai Sakit': dateOnly(item.sampai),
      'Lama Sakit (Hari)': item.jumlah_hari,
    ...
    const colWidths =
      excelData.length > 0
        ...
        : [];
    ws['!cols'] = colWidths;
    XLSX.utils.book_append_sheet(wb, ws, 'Data Izin Sakit');

    const currentDate = new Date().toISOString().split('T')[0];
    let filename = 'Data_Izin_Sakit';
...
export default HistorySakitKeHR;

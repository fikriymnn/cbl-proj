...
const JOApproval: React.FC = () => {
  ...
  const handleApprove = async (id: number) => {
    if (window.confirm('Apakah Anda yakin ingin menyetujui JO ini?')) {
      setActionLoading((prev) => ({ ...prev, [id]: true }));
      try {
        const url = `${import.meta.env.VITE_API_LINK}/ppic/jo/approve/${id}`;
        await axios.put(
          url,
          {},
          {
        ...
        await fetchJOData();
        alert('JO berhasil disetujui!');
      } catch (error: any) {
        console.error('Error approving JO:', error);
        alert(
          error.response?.data?.message ||
            'Gagal menyetujui JO. Silakan coba lagi.',
        );
      } finally {
  ...
  const handleReject = async (id: number) => {
    if (window.confirm('Apakah Anda yakin ingin menolak JO ini?')) {
      setActionLoading((prev) => ({ ...prev, [id]: true }));
      try {
        const url = `${import.meta.env.VITE_API_LINK}/ppic/jo/reject/${id}`;
        await axios.put(
          url,
          {},
          {
        ...
        await fetchJOData();
        alert('JO berhasil ditolak!');
      } catch (error: any) {
        console.error('Error rejecting JO:', error);
        alert(
          error.response?.data?.message ||
            'Gagal menolak JO. Silakan coba lagi.',
        );
      } finally {
        setActionLoading((prev) => ({ ...prev, [id]: false }));
      }
                        ...
                        {item.status_proses === 'request to kabag' ? (
                          <div className="flex flex-col gap-2">
                            <button
                              ...
                              className="text-white bg-blue-500 hover:bg-blue-600 px-3 py-1 rounded text-xs transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                              title="EDIT JO"
                            ...
                            <button
                              ...
                              className="bg-green-500 hover:bg-green-600 text-white px-3 py-1 rounded text-xs transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-1"
                              title="Approve JO"
                              disabled={actionLoading[item.id]}
                            >
                            ...
                            <button
                              ...
                              className="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded text-xs transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-1"
                              title="Reject JO"
                              disabled={actionLoading[item.id]}
                            >
                              {actionLoading[item.id] ? (

...
function TampilanDailyJO() {
  ...
  const getConflictingJobs = (hour: string, machine: string) => {
    return mapData.filter(
      (item: any) => item.jam === hour && item.mesin === machine,
    );
  };
      ...
      <div className="min-w-[700px] bg-white rounded-xl flex gap-1 px-4 py-4">
        <>
          <div className="flex w-full flex-col">
                ...
                {hours.map((hour, rowIndex) => (
                  ...
                  >
                    ...
                    {machineList.map((machine, colIndex) => {
                      ...
                      if (isRealConflict) cellBgColor = 'bg-red-300';
            ...
            {hoveredJobOrder && (
              <div className="fixed bottom-4 right-4 bg-white p-4 rounded-md border-2 border-black z-50">

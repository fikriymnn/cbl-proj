...
const ListPerubahan: React.FC = () => {
  ...
  const [selectedInvoiceForPerubahan, setSelectedInvoiceForPerubahan] =
    useState<PerubahanInvoiceItem | null>(null);
  ...
  const handlePerubahan = async (item: PerubahanInvoiceItem) => {
    try {
      ...
      setSelectedInvoiceForPerubahan(detail);
  ...
  const handleSubmitPerubahan = async (formData: PerubahanFormData) => {
    ...
    try {
      ...
      console.error('Error submitting perubahan:', err);
  ...
  const handleRequest = async (item: PerubahanInvoiceItem) => {
    try {
      const res = await axios.put(
        `${import.meta.env.VITE_API_LINK}/perubahanInvoice/request/${item.id}`,
        {},
        { withCredentials: true },
      );

      if (res.data.success) {
        alert('Perubahan invoice berhasil direquest!');
        fetchInvoiceData();
      }
    } catch (err) {
      console.error('Error requesting perubahan invoice:', err);
      alert('Gagal request perubahan invoice.');
    }
  };
    ...
    <div className="">
            ...
            <div className="flex gap-2 mt-3">
              {(item.status === 'draft' || item.status === 'rejected') && (
                <>
                  <button
                    onClick={() => handlePerubahan(item)}
                    className="flex-1 px-3 py-2 text-xs bg-orange-600 text-white rounded"
                  >
                    Edit
                  </button>
      ...
      {isPerubahanModalOpen && perubahanFormData && (
        <PerubahanModal
          ...
          onClose={() => setIsPerubahanModalOpen(false)}

...
const EstimasiKurangQtyMarketing: React.FC = () => {
  ...
  const fetchListData = async (): Promise<void> => {
    ...
    try {
      ...
      console.error('Error fetching estimasi kurang qty (Marketing):', error);
      setListData([]);
    } finally {
      setLoading(false);
    }
  ...
  const openDetailModal = async (id: number): Promise<void> => {
    ...
    try {
      ...
      console.error(
        'Error fetching estimasi kurang qty detail (Marketing):',
        error,
      );
  ...
  return (
    <div className="">
      ...
      {/* Detail Modal (read-only, no approve action) */}
      {showModal && (
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4">
              {detailLoading ? (
                ...
                <div className="space-y-4">
                  ...
                  {/* Note - view only, no editing */}
                  {selectedDetail.note && (
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">
                        Note
                      </label>
                      <p className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg bg-gray-50 text-gray-700 whitespace-pre-wrap">
                        {selectedDetail.note}
                      </p>
                    </div>
                  )}
...
export default EstimasiKurangQtyMarketing;

...
function HistoryCutiKeHR() {
  ...
  async function getMe() {
    ...
    try {
      ...
      getCuti(res?.data.karyawan.biodata_karyawan[0]?.id_department);
      console.log('getme', res.data);
    } catch (error: any) {
      console.log(error.data.msg);
    }
  }
  async function getCuti(idDept: any) {
  ...
  const handleResetFilters = () => {
    ...
    setTimeout(() => getCuti(idPengaju), 100);
  };

  const exportToExcel = () => {
    if (!cuti?.data || cuti.data.length === 0) {
      alert('No data to export');
      return;
    }

    // Prepare data for Excel
    const excelData = cuti.data.map((item: any, index: number) => ({
      No: index + 1,
      ...
      'Tipe Cuti': item.tipe_cuti || '',
      'Lama Cuti (Hari)': item.jumlah_hari,
      'Alasan Cuti': item.alasan_cuti || '',

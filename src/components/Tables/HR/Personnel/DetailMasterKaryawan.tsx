...
export default function DetailMasterKaryawanIsi() {
  ...
  const [statusFilter, setStatusFilter] = useState<'Terlambat' | 'Tepat Waktu'>(
    'Terlambat',
  );
  const [isLoading, setIsLoading] = useState(false);
  const [karyawan, setKaryawan] = useState<any>(null);

  // Format date function for API calls
  ...
  async function getKaryawan() {
    ...
    try {
      ...
      // Extract the timestamp date and format it correctly
      let startDate = today; // Default to today
      ...
      // Call the other API functions with the formatted start date
      await Promise.all([getAbsen(startDate), getRekapAbsen(startDate)]);
  ...
  // Process attendance data when rekapAbsen changes
  useEffect(() => {
    if (rekapAbsen?.data && id) {
      const parsedId = parseInt(id as string, 10);

      // Find the employee data by looking through all entries and their absensi arrays
      let userAttendance: any[] = [];
      rekapAbsen.data.forEach((item: any) => {
        if (item.absensi && Array.isArray(item.absensi)) {
          const matchingEntries = item.absensi.filter(
            (entry: any) => entry.userid === parsedId,
          );
          if (matchingEntries.length > 0) {
            userAttendance = [...userAttendance, ...matchingEntries];
          }
        }
      });

      // Filter for the status (Terlambat or Tepat Waktu)
      const filtered = userAttendance.filter((item: any) => {
        if (statusFilter === 'Terlambat') {
          return item.status_masuk && item.status_masuk.trim() === 'Terlambat';
        } else {
          return (
            item.status_masuk && item.status_masuk.trim() === 'Tepat Waktu'
          );
        }
      });
      ...
      filtered.sort((a: any, b: any) => {
        const dateA = new Date(a.waktu_masuk);
        const dateB = new Date(b.waktu_masuk);
        return dateB.getTime() - dateA.getTime();
      });

      setFilteredAttendance(filtered);
    }
  }, [rekapAbsen, id, statusFilter]);

  // Toggle between "Terlambat" and "Tepat Waktu"
  const toggleStatusFilter = () => {
    setStatusFilter((prev) =>
      prev === 'Terlambat' ? 'Tepat Waktu' : 'Terlambat',
    );
    setShowAll(false); // Reset the show all state when toggling
  };
      ...
      <div className=" bg-white rounded-xl">
        <div className=" w-full h-full flex gap-1 flex-col border-b-8 border-[#D8EAFF] px-6 py-[2%] justify-between">
          <div className="flex flex-col gap-1">
            <label htmlFor="" className="text-black text-sm font-semibold">
              Nama Karyawan
            </label>
            <label htmlFor="" className="text-[#636363] text-xl font-normal ">
              {karyawan?.data?.name}
            </label>
          </div>
          <div className="flex flex-col gap-1">
          ...
          <DetailTabMasterKaryawan>
            <>
              <div className="bg-[#eeeeee] px-6 py-2">
                <label
                  htmlFor=""
                  className="text-blue-400 text-sm font-normal "
                >
                  BIODATA
                </label>
              </div>
          ...
          <DetailTabMasterKaryawan>
            <>
              <div className="flex flex-col w-full">
                ...
                <div className="grid grid-cols-2 px-6 py-3 border-b-8 border-blue-100">
                  <div className="flex flex-col gap-2">
                    <div className="flex flex-col gap-1">
              ...
              <div className="grid grid-cols-12 gap-1 px-6 py-2 border-b-4 border-blue-100">
                ...
                <label className="text-black text-sm font-semibold col-span-2">
                  {statusFilter === 'Terlambat' ? 'TERLAMBAT' : 'STATUS'}
                </label>
              </div>
            ...
            <>
              <div className="border-b-8 border-[#D8EAFF]">
                ...
                <div className="grid grid-cols-12 gap-1 px-6 py-2 border-b-4 border-[#D8EAFF]">
                  ...
                  <label
                    htmlFor=""
                    className="text-black text-sm font-semibold col-span-3"
                  >
                    TANGGAL
                  </label>
                  <label
                ...
                <div className="grid grid-cols-12 gap-1 px-6 py-2 ">
                  ...
                  <label
                    htmlFor=""
                    className="text-stone-500 text-sm font-semibold col-span-3"
                  >
                    12 - DESEMBER - 2024
                  </label>
                  <label
                    htmlFor=""
                    className="text-red-300 text-sm font-semibold col-span-3"
                  >
                    OPERATOR
                  </label>
                  <label
                    htmlFor=""
                    className="text-blue-300 text-sm font-semibold col-span-3"
                  >
                    SENIOR OPERATOR
                  </label>
                </div>
          ...
          <DetailTabMasterKaryawan>
            <>
              <div className="border-b-8 border-[#D8EAFF]">
                ...
                <div className="grid grid-cols-12 gap-1 px-6 py-2 border-b-4 border-[#D8EAFF]">
                  <label htmlFor="" className='"text-[#636363] text-xl'>
                    Rp.
              ...
              <div className="flex flex-col gap-1 px-4 py-1 w-full">
                <div className="grid grid-cols-12 gap-1 px-6 py-2 border-b-4 border-[#D8EAFF]">
                  <label
                    htmlFor=""
                    className="text-black text-sm font-semibold col-span-4"
                  >
                    Nama Potongan
                  </label>
                  <label
                  ...
                  (data: any, i: any) => (
                    <>
                      <div className="grid grid-cols-12 gap-1 px-6 py-2 ">
                        <label
                          htmlFor=""
                          className="text-stone-500 text-sm font-semibold col-span-4"
                        >
                          {data.nama_potongan}
                        </label>
                        <label
            ...
            <>
              ...
              <div className="">
                ...
                <div className="grid grid-cols-12 gap-1 px-6 py-2 border-b-4 border-[#D8EAFF]">
                  ...
                  <label
                    htmlFor=""
                    className="text-black text-sm font-semibold col-span-2"
                  >
                    TAHUN LULUS
                  </label>
                  <label
                ...
                <div className="grid grid-cols-12 gap-1 px-6 py-2 ">
                  ...
                  <label
                    htmlFor=""
                    className="text-stone-500 text-sm font-semibold col-span-2"
                  >
                    2004
                  </label>
                  <label
            ...
            <>
              <div className="border-b-8 border-[#D8EAFF]">
                ...
                <div className="grid grid-cols-12 gap-1 px-6 py-2 border-b-4 border-[#D8EAFF]">
                  <label htmlFor="" className='"text-[#636363] text-xl'>
                    Rp.
                    {karyawan?.data?.biodata_karyawan[0]?.limit_pinjaman ==
                      null ||
                    karyawan?.data?.biodata_karyawan[0]?.limit_pinjaman == 0
                      ? '-'
                      : formatInteger(
                          karyawan?.data?.biodata_karyawan[0]?.limit_pinjaman,
                        )}

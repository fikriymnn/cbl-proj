...
function StaffKalenderKerja() {
  ...
  async function getKaryawanJadwal() {
    ...
    try {
      ...
      console.log('Staff Jadwal:', res.data);
      console.log('Produksi Jadwal:', res2.data);
    } catch (error: any) {
      setIsLoading(false);
      console.log(error);
    }
  ...
  async function getBiasaJadwal(start: any, end: any) {
    const url = `${import.meta.env.VITE_API_LINK}/hr/jadwalKaryawan`;
  ...
  async function deleteMingguJadwal(
    thn: any,
    namaJadwal: any,
    jenisKaryawan: any,
  ) {
    if (
      window.confirm(
        `Apakah Anda yakin akan menghapus hari Minggu di Tahun ${thn} dengan keterangan ${namaJadwal} dengan tipe karyawan ${jenisKaryawan}`,
      )
    ) {
      const url = `${import.meta.env.VITE_API_LINK}/hr/jadwalKaryawanSatuTahun`;
  ...
  async function deleteBiasaJadwal(id: any, tgl: any, ket: any) {
    if (
      window.confirm(
        `Apakah Anda yakin akan menghapus hari libur di tanggal ${tgl} ini dengan keterangan ${ket}`,
      )
    ) {
      const url = `${import.meta.env.VITE_API_LINK}/hr/jadwalKaryawan/${id}`;
  ...
  async function postSabtuMinggu() {
    if (
      window.confirm(
        `Apakah Anda yakin akan menambah hari libur 1 tahun di ${tahun} dengan hari ${hari} dengan jenis karyawan ${jenisKaryawan}`,
      )
    ) {
      const url = `${import.meta.env.VITE_API_LINK}/hr/jadwalKaryawanSatuTahun`;
      try {
  ...
  async function postSatuHari() {
    if (
      window.confirm(
        `Apakah Anda yakin akan menambah hari libur Pada tanggal ${tgl} dengan nama ${namaJadwal}`,
      )
    ) {
      const url = `${import.meta.env.VITE_API_LINK}/hr/jadwalKaryawan`;
      try {
      ...
      <main>
                ...
                >
                  <>
                    <div className="flex flex-col gap-2 py-2 px-4">
                      ...
                      <button
                        onClick={() =>
                          deleteMingguJadwal(thn, jadwalName, jeka)
                        }
                    ...
                    <div className="max-h-[500px] overflow-y-scroll">
                      {biasaJadwal?.data?.map((data: any, i: number) => (
                        <>
                          ...
                          >
                            <p className="text-[#646464] text-sm  ">{i + 1}</p>
        ...
        <div className=" w-full bg-white rounded-md flex flex-col border-b-8 border-[#D8EAFF]   ">
          <div className="grid grid-cols-12 items-center px-3 py-4  gap-2 w-full border-b-8 border-[#D8EAFF]">
            <label className="text-neutral-500 text-sm font-semibold">No</label>

...
function HistoryPulangCepat() {
  ...
  const handleResetFilters = () => {
    ...
    setTimeout(() => getPulangCepat(), 100);
  };

  const exportToExcel = () => {
    if (!pulangCepat?.data || pulangCepat.data.length === 0) {
      alert('No data to export');
      return;
    }

    // Prepare data for Excel
    const excelData = pulangCepat.data.map((item: any, index: number) => ({
      No: index + 1,
      ...
      'Tanggal Pulang Cepat': dateOnly(item.tanggal),
      'Jam Pulang': item.jam_pulang || '',
      'Tipe Izin': item.type_izin || '',
      Status: item.status?.toUpperCase(),
      'Yang Menyetujui': item.karyawan_hr?.name || '',
      'Alasan Pulang Cepat': item.alasan || '',
    ...
    const colWidths =
      excelData.length > 0
        ...
        : [];
    ws['!cols'] = colWidths;
    XLSX.utils.book_append_sheet(wb, ws, 'Data Pulang Cepat');

    const currentDate = new Date().toISOString().split('T')[0];
    let filename = 'Data_Pulang_Cepat';
  ...
  async function hapusCuti(id: any, name: any) {
    if (
      window.confirm(
        `Apakah Anda yakin ingin Membatalkan Pulang Cepat Untuk Karyawan  ${name}    ?`,
      )
    ) {
      const url = `${
            ...
            {pulangCepat?.data?.map((data: any, i: any) => {
              return (
                ...
                >
                  ...
                  <div className="justify-end flex flex-col col-span-2 ">
                    <button
...
export default HistoryPulangCepat;

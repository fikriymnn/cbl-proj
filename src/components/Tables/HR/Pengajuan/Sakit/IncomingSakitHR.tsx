...
function IncomingSakitHR() {
    ...
    async function approveSakit(id: any, index: any) {
        ...
        const url = `${import.meta.env.VITE_API_LINK}/hr/pengajuanSakit/approve/${id}`;
        try {
            setIsLoading(true)
    ...
    async function rejectSakit(id: any, index: number) {
        ...
        if (window.confirm('Apakah Anda yakin ingin menolak pengajuan sakit ini?')) {
            const url = `${import.meta.env.VITE_API_LINK}/hr/pengajuanSakit/reject/${id}`;
            try {
                setIsLoading(true)
                const res = await axios.put(url,
                    {

...
function IncomingSP() {
    ...
    async function approveIzin(id: any, index: any) {
        ...
        const url = `${import.meta.env.VITE_API_LINK}/hr/pengajuanSP/approve/${id}`;
        try {
            setIsLoading(true)
    ...
    async function rejectIzin(id: any, index: number) {
        ...
        if (window.confirm('Apakah Anda yakin ingin menolak pengajuan SP ini?')) {
            const url = `${import.meta.env.VITE_API_LINK}/hr/pengajuanSP/reject/${id}`;
            try {
                setIsLoading(true)
                const res = await axios.put(url,
                    {

...
function DiProsesIzinHR() {
  ...
  const exportToExcel = async () => {
    try {
      ...
      const excelData = exportData.data.map((item: any, index: number) => ({
        ...
        'Lama Izin (Hari)': item.jumlah_hari,
        'Alasan Izin': item.alasan_izin || '',
  ...
  async function hapusCuti(id: any, dari: any, sampai: any, name: any) {
    if (
      window.confirm(
        `Apakah Anda yakin ingin Membatalkan Izin Untuk Karyawan  ${name} pada tanggal ${dari} - ${sampai}   ?`,
      )
    ) {
      const url = `${

...
function IncomingMangkirHR() {
    ...
    async function approveMangkir(id: any, index: any) {
        if (catatanHr == null) {
            alert('Catatan Wajib Diisi');
            return;
        }
        const url = `${import.meta.env.VITE_API_LINK}/hr/pengajuanMangkir/approve/${id}`;
        try {
            setIsLoading(true)
    ...
    async function rejectMangkir(id: any, index: number) {
        if (catatanHr == null) {
            alert('Catatan Wajib Diisi');
            return;
        }
        if (window.confirm('Apakah Anda yakin ingin menolak pengajuan mangkir ini?')) {
            const url = `${import.meta.env.VITE_API_LINK}/hr/pengajuanMangkir/reject/${id}`;
            try {
                setIsLoading(true)
                const res = await axios.put(url,
                    {
                                                        ...
                                                        <ModalKosongan
                                                            ...
                                                            <>
                                                                ...
                                                                <div className='flex gap-2 w-full px-4 pt-1'>
                                                                    <button
                                                                        disabled={isLoading}
                                                                        onClick={() => approveMangkir(data.id, i)}
                                                                    ...
                                                                    <button
                                                                        disabled={isLoading}
                                                                        onClick={() => rejectMangkir(data.id, i)}
...
export default IncomingMangkirHR;

...
const EnhancedEmployeeFilter: React.FC<FilterProps> = ({
  ...
  useEffect(() => {
    ...
    const filteredResult = originalData.filter((item: any) => {
      ...
      const divisiValue = item.nama_divisi || '';

      const tipeKaryawanValue =
        item.karyawan?.biodata_karyawan[0]?.tipe_karyawan || '';
      const tipePenggajiannValue =
        item.karyawan?.biodata_karyawan[0]?.tipe_penggajian || '';

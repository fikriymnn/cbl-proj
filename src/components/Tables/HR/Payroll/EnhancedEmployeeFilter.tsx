...
const EnhancedEmployeeFilter: React.FC<FilterProps> = ({
    ...
    useEffect(() => {
        ...
        const filteredResult = originalData.filter((item: any) => {
            const departmentValue = item.karyawan?.biodata_karyawan[0]?.department?.nama_department || '';
            const divisiValue = item.karyawan?.biodata_karyawan[0]?.divisi?.nama_divisi || '';
            ...
            const matchDepartment = !filters.nama_department ||
                departmentValue === filters.nama_department;

            const matchDivisi = !filters.nama_divisi ||
                divisiValue === filters.nama_divisi;
            ...
            const matchNik = !filters.nik ||
                nikValue.toString().includes(filters.nik);
                ...
                <div className="bg-blue-50 p-4 rounded-lg shadow">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div>
                            ...
                            <p className="text-sm font-semibold">{convertTimeStampToDate(summaryStats.periodFrom)}</p>
                        </div>
                        <div>
                            <p className="text-sm font-medium text-gray-500">Periode Sampai</p>
                            <p className="text-sm font-semibold">{convertTimeStampToDate(summaryStats.periodTo)}</p>
                        </div>
                        <div>
                            <p className="text-sm font-medium text-gray-500">Tanggal Bayar</p>
                            <p className="text-sm font-semibold">{convertTimeStampToDate(summaryStats.paymentDate)}</p>
                        </div>
                        <div>
                            <p className="text-sm font-medium text-gray-500">Total</p>
                            <p className="text-sm font-semibold">Rp. {formatInteger(summaryStats.total)}</p>
                        </div>
                    </div>
                </div>
            )}

...
function PayrollBulanan() {
  ...
  async function getPayrollBulanan(
    dateFrom1: any,
    dateTo1: any,
    id_karyawan: any,
  ) {
  ...
  async function bayarPayrollBulanan(
    dateFrom1: any,
    dateTo1: any,
    id_karyawan: any,
    data_payroll: any,
                          ...
                          <div className="flex flex-col gap-1 py-4">
                              ...
                              <button
                                ...
                                onClick={() => {
                                  getPayrollBulanan(
                                    dateFrom,
                                    dateTo,
                                    data.biodata_karyawan[0]?.id_karyawan,
                                  );
                                }}
                            ...
                            <div className="grid grid-cols-2 items-center py-2  border-b-8 border-[#D8EAFF]">
                              <div className="text-sm text-neutral-500  ">
                                {payMonth?.data?.summaryPayroll?.potonganIzin
                                  ?.length != 0 ? (
                                  <div className="flex">
                                    <p>Potongan Izin :</p>
                                    <div>
                                      {payMonth?.data?.summaryPayroll?.potonganIzin?.map(
                                        (
                                          dataPotonganIzin: any,
                                          iRincian: number,
                                        ) => {
                                          return (
                                            <p className="text-sm flex flex-col">
                                              {`${iRincian + 1}. ${
                                                dataPotonganIzin.label
                                              } : ${dataPotonganIzin.nilai} = ${
                                                dataPotonganIzin.total
                                              }`}
                                            </p>
                                          );
                                        },
                                ...
                                {payMonth?.data?.summaryPayroll?.potonganSakit
                                  ?.length != 0 ? (
                                  <div className="flex">
                                    <p>Potongan Sakit :</p>
                                    <div>
                                      {payMonth?.data?.summaryPayroll?.potonganSakit?.map(
                                        (dataSakit: any, iSakit: number) => {
                                          return (
                                            <p className="text-sm flex flex-col">
                                              {`${iSakit + 1}. ${
                                                dataSakit.label
                                ...
                                {payMonth?.data?.summaryPayroll?.potonganMangkir
                                  .length != 0 ? (
                                  <div className="flex">
                                    <p>Potongan Mangkir :</p>
                                    <div>
                                      {payMonth?.data?.summaryPayroll?.potonganMangkir?.map(
                                        (
                                          dataMangkir: any,
                                          iMangkir: number,
                                        ) => {
                                ...
                                <div>
                                  {'TMK : ' +
                                    payMonth?.data?.summaryPayroll?.tmk}
                                </div>
                                <div>
                                  {'Gaji : ' +
                                    payMonth?.data?.summaryPayroll?.gaji}
                                </div>
                              </div>
                              <div className="flex flex-col justify-end w-full">
                                <div>
                                  {`Total Gaji : Rp.  ${
                                    payMonth?.data?.summaryPayroll?.total +
                                    parseInt(insentif)
                                  }`}
                                </div>
                              </div>

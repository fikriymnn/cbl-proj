...
export default function TabOSHR() {
  ...
  async function getMe() {
    const url = `${import.meta.env.VITE_API_LINK}/hr/outstandingIncoming`;
    try {
        ...
        <AppBar position="static" className="">
          <ThemeProvider theme={theme}>
            ...
            >
              ...
              <Tab
                label={`Absensi ${
                  task?.otsAbsensi == 0 ? '' : '(' + task?.otsAbsensi + ')'
                }`}
                {...a11yProps(1)}
              />
              <Tab
                label={`Status Karyawan ${
                  task?.otsKaryawan == 0 ? '' : '(' + task?.otsKaryawan + ')'
                }`}
                {...a11yProps(2)}
              />
              <Tab
                label={`Ketidaksesuaian SPL ${
                  task?.otsKetidaksesuaianSpl == 0
                    ? ''
                    : '(' + task?.otsKetidaksesuaianSpl + ')'
                }`}
                {...a11yProps(3)}
              />

...
function ListIncoming2() {
  ...
  async function getInspection() {
    ...
    }/qc/cs/inspeksiBahan?status=incoming`;
    const url2 = `${
      import.meta.env.VITE_API_LINK
    }/qc/cs/inspeksiChemical?status=incoming`;
    try {
                ...
                .map((data: any, i: any) => {
                  ...
                  return (
                    <section className=" grid grid-cols-12 px-4 items-center gap-2 border-b-8 border-[#D8EAFF] ">
                      ...
                      <div className="flex flex-col    bg-white">
                        <p className="text-stone-500 text-sm font-medium ">
                          {data.tipe == 'chemical' ? 'Chemical' : 'Bahan'}
                        </p>
                      </div>
                      <div className="flex flex-col  justify-end  items-end">
                        {data.tipe == 'chemical' ? (
                          <>
                            <Link
                              to={`/qc/qualityinspectionChemical/list/${data.id}`}
                            >
                              <button
                          ...
                          <>
                            <Link
                              to={`/qc/inspection/chemical/list/${data.id}`}
                            >
                              <button

...
function CheckSheetLemPeriode() {
  ...
  // Adjusted handleChangePointHasil for Lem
  const handleChangePointHasil = (
    ...
    const { name, value } = e.target;
    const onchangeVal: any = LemMesinPeriode; // Changed from cetakMesinPeriode
    ...
    if (name === 'kode_lkh') {
      ...
      if (selectedKendala) {
        onchangeVal.inspeksi_lem_periode[0].inspeksi_lem_periode_point[
          i
        ].inspeksi_lem_periode_defect[ii]['kode_lkh'] = value;
        onchangeVal.inspeksi_lem_periode[0].inspeksi_lem_periode_point[
          i
        ].inspeksi_lem_periode_defect[ii]['masalah_lkh'] =
    ...
    setLemMesinPeriode(onchangeVal); // Changed from setCetakMesinPeriode
  };

  // Adjusted image upload handler for Lem
  const handleImageUpload = async (file: File, i: number, ii: number) => {
    try {
      ...
      onchangeVal.inspeksi_lem_periode[0].inspeksi_lem_periode_point[
        i
      ].inspeksi_lem_periode_defect[ii]['file'] = fileName;
      setLemMesinPeriode(onchangeVal); // Changed from setCetakMesinPeriode
    } catch (error) {
  ...
  // Adjusted image delete handler for Lem
  const handleImageDelete = async (i: number, ii: number) => {
    try {
      const currentFile =
        LemMesinPeriode.inspeksi_lem_periode[0].inspeksi_lem_periode_point[i] // Changed from cetakMesinPeriode
          .inspeksi_lem_periode_defect[ii]?.file;
  ...
  async function updateDefect(defectId: number) {
    const url = `${
      import.meta.env.VITE_API_LINK
    }/qc/cs/inspeksiLemPeriodePoint/updateDefect/${defectId}`;
    try {
      setIsLoading(true);
      await axios.put(
        url,
                                        ...
                                        data.status == 'on progress' && (
                                          <div className="space-y-2">
                                            ...
                                            <div className="space-y-2">
                                              ...
                                              {LemMesinPeriode
                                                .inspeksi_lem_periode[0]
                                                ?.inspeksi_lem_periode_point[
                                                index
                                              ]?.inspeksi_lem_periode_defect[i]
                                                ?.file ? (
                                                <div className="space-y-2">
                          ...
                          <div className="flex flex-col gap-4 p-4">
                            ...
                            <div>
                              ...
                              <input
                                ...
                                type="text"
                                className="w-full p-2 border border-gray-300 rounded-md focus:border-blue-5 focus:outline-none"
                              />
                            </div>

...
function CheckSheetCetakPeriode() {
  ...
  // Updated handleChangePointHasil function
  const handleChangePointHasil = (
    ...
    if (name === 'kode_lkh') {
      ...
      if (selectedKendala) {
        onchangeVal.inspeksi_cetak_periode[0].inspeksi_cetak_periode_point[
          i
        ].inspeksi_cetak_periode_defect[ii]['kode_lkh'] = value;
        onchangeVal.inspeksi_cetak_periode[0].inspeksi_cetak_periode_point[
          i
        ].inspeksi_cetak_periode_defect[ii]['masalah_lkh'] =
  ...
  const handleImageUpload = async (file: File, i: number, ii: number) => {
    try {
      ...
      onchangeVal.inspeksi_cetak_periode[0].inspeksi_cetak_periode_point[
        i
      ].inspeksi_cetak_periode_defect[ii]['file'] = fileName;
      setCetakMesinPeriode(onchangeVal);
    } catch (error) {
  ...
  const handleImageDelete = async (i: number, ii: number) => {
    try {
      const currentFile =
        cetakMesinPeriode.inspeksi_cetak_periode[0]
          .inspeksi_cetak_periode_point[i].inspeksi_cetak_periode_defect[ii]
          ?.file;
      ...
      if (currentFile) {
        ...
        onchangeVal.inspeksi_cetak_periode[0].inspeksi_cetak_periode_point[
          i
        ].inspeksi_cetak_periode_defect[ii]['file'] = '';
        setCetakMesinPeriode(onchangeVal);
      }
    } catch (error) {
  ...
  async function updateDefect(defectId: number) {
    const url = `${
      import.meta.env.VITE_API_LINK
    }/qc/cs/inspeksiCetakPeriodePoint/updateDefect/${defectId}`;
    try {
      setIsLoading(true);
      await axios.put(
        url,
    ...
    <>
      {!isMobile && (
        <main className="overflow-x-hidden">
                        ...
                        <div className="grid grid-cols-12  border-b-8 border-[#D8EAFF]">
                          ...
                          <div className="flex flex-col gap-2 col-span-2  py-4">
                            ...
                            <label className="text-neutral-500 text-sm font-semibold">
                              :{' '}
                              {formatInteger(
                                parseInt(cetakMesinPeriodeHistory?.jumlah_druk),
                              )}
                            </label>
                            <label className="text-neutral-500 text-sm font-semibold">
                              :{' '}
                                      ...
                                      data.status == 'on progress' && (
                                        <div className="space-y-2">
                                          ...
                                          <div className="space-y-2">
                                            ...
                                            {cetakMesinPeriode
                                              .inspeksi_cetak_periode[0]
                                              ?.inspeksi_cetak_periode_point[
                                              index
                                            ]?.inspeksi_cetak_periode_defect[i]
                                              ?.file ? (
                                              <div className="space-y-2">
              ...
              <div className="px-4 py-4">
                {(() => {
                  ...
                  const allNotOkDefects =
                    cetakMesinPeriode?.inspeksi_cetak_periode[0]?.inspeksi_cetak_periode_point?.flatMap(
                      (period: any) =>
                        period.inspeksi_cetak_periode_defect.filter(
                  ...
                  // Grand total is now sum of all (defect * UP) calculations
                            ...
                            <div className="flex justify-between items-center">
                              <div>
                                <div className="text-lg font-bold">
                                  🚨 TOTAL TEMUAN DEFECTS (CETAK)
                                </div>
                                <div className="text-sm opacity-90">
                                  {cetakMesinPeriode?.inspeksi_cetak_periode[0]
                                    ?.inspeksi_cetak_periode_point?.length ||
                                    0}{' '}
                                  Periode
                                </div>
                              </div>
                ...
                <div className="flex flex-col gap-2">
                  <div className="flex gap-2">
                    ...
                    <input
                      readOnly
                      value={
                        cetakMesinPeriode?.inspeksi_cetak_periode[0]?.sample_1
                      }
                      type="text"
                      className="border-2 border-stroke w-[40%] rounded-sm col-span-2"
                    />
                ...
                <div className="flex gap-2 flex-col">
                  <div className="flex gap-2">
                    ...
                    <input
                      readOnly
                      value={
                        cetakMesinPeriode?.inspeksi_cetak_periode[0]?.sample_2
                      }
                      type="text"
                      className="border-2 border-stroke w-[40%] rounded-sm col-span-2"
                    />
                ...
                <div className="flex gap-2 flex-col">
                  <div className="flex gap-2">
                    ...
                    <input
                      readOnly
                      value={
                        cetakMesinPeriode?.inspeksi_cetak_periode[0]?.sample_3
                      }
                      type="text"
                      className="border-2 border-stroke w-[40%] rounded-sm col-span-2"
                    />
          ...
          <div className="flex w-full min-w-[700px] border-b-8 items-center border-[#D8EAFF] px-4 py-4 gap-5 bg-white rounded-b-xl mt-2"></div>
        </main>
      )}
      {/* Edit Defect Modal */}
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setEditForm((prev: any) => ({
                            ...
                            // reset not-ok fields when switching away
                            ...(e.target.value !== 'not ok' && {
                              jumlah_defect: '',
                              jumlah_up_defect: '',
                              kode_lkh: '',

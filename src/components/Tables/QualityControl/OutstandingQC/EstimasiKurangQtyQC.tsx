...
const EstimasiKurangQtyQC: React.FC = () => {
  ...
  const fetchListData = async (): Promise<void> => {
    const url = `${import.meta.env.VITE_API_LINK}/qc/estimasiKurangQty`;
    try {
      setLoading(true);
      ...
      console.error('Error fetching estimasi kurang qty (QC):', error);
      setListData([]);
    } finally {
      setLoading(false);
    }
  ...
  const openDetailModal = async (id: number): Promise<void> => {
    ...
    const url = `${import.meta.env.VITE_API_LINK}/qc/estimasiKurangQty/${id}`;
    try {
      const res: AxiosResponse<EstimasiKurangQtyDetailResponse> =
        await axios.get(url, { withCredentials: true });
      setSelectedDetail(res.data.data);
    } catch (error) {
      console.error('Error fetching estimasi kurang qty detail (QC):', error);
  ...
  const handleApprove = async (id: number): Promise<void> => {
    ...
    try {
      ...
      console.error('Error approving estimasi kurang qty (QC):', error);
...
export default EstimasiKurangQtyQC;

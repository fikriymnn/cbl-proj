...
const ModalEditAnalisisMaster = ({ children, isOpen, onClose, idAnalisis, data }:
    {
        children: any, isOpen: any, onClose: any, idAnalisis: number, data: any
    ...
    const [kode, setKode] = useState(data)
    ...
    //change value Mesin pm1
    const handleChangekategori = (e: any,) => {
        const { value } = e.target;
        const onchangeVal: any = kategori;
        onchangeVal.bagian_analisis = value;
        setKategori(onchangeVal);
    };
    const handleChangeKode = (e: any,) => {
        const { value } = e.target;
        const onchangeVal: any = kode;
        onchangeVal.kode_analisis = value;
        setKode(onchangeVal);
    };
    const handleChangeAnalisis = (e: any,) => {
        const { value } = e.target;
        const onchangeVal: any = analisis;
        onchangeVal.nama_analisis = value;
        setAnalisis(onchangeVal);
    };
    async function submitEDitMesin(id: number) {
        const url = `${import.meta.env.VITE_API_LINK}/master/kodeAnalisis/${id}`;

        try {
            const res = await axios.put(
                url,
                {
                    bagian_analisis: kategori.bagian_analisis,
                    kode_analisis: kategori.kode_analisis,
                    nama_analisis: kategori.nama_analisis
        ...
        <div className="fixed z-50 inset-0 h-full backdrop-blur-sm bg-white/10 p-4 md:p-8 flex justify-center items-center">
            <div className="w-full max-w-3xl bg-white rounded-xl shadow-md max-h-screen ">
                <div className="flex w-full items-center pt-4 px-3">
                    ...
                    <label className='flex w-11/12 text-blue-700 text-sm font-bold '>Form Edit Master Analisis</label>
                ...
                <div className="flex w-full flex-col pt-7 ">
                    <>
                        ...
                        <div className="flex w-full flex-row gap-4 pt-3 px-4">
                            <label className="text-black text-xs font-bold w-30">
                                Kategori Analisis
                            </label>
                            <div className="flex w-full">
                                <input
                                    name="nama_mesin"
                                    defaultValue={kategori.bagian_analisis}
                        ...
                        <div className="flex w-full flex-row gap-4 pt-3 px-4">
                            ...
                            <div className="flex w-full">
                                <input
                                    name="lokasi_mesin"
                                    defaultValue={kategori.kode_analisis}
                                    onChange={(e) => handleChangeKode(e)}
                                    type="text"
                                    className=" w-[387px] h-10 border-2 border-stroke rounded-md"
                                />
                            </div>

                        </div><div className="flex w-full flex-row gap-4 pt-3 px-4">
                            <label className="text-black text-xs font-bold w-30">
                                Nama Analisis
                            </label>
                            <div className="flex w-full">
                                <input
                                    name="bagian_mesin"
                                    defaultValue={kategori.nama_analisis}
                                    onChange={(e) => handleChangeAnalisis(e)}
                                    type="text"
                                    className=" w-[387px] h-10 border-2 border-stroke rounded-md"
                                />
                            </div>
                        ...
                        <div className="flex w-full px-6 py-3 justify-end">
                            <button
                                onClick={() => submitEDitMesin(idAnalisis)}
...
export default ModalEditAnalisisMaster;
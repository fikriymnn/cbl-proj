              ...
              {loading ? (
                ...
                <>
                  {filteredData?.map((data: any, index) => {
                    ...
                    const tglPasang = new Date(data.tgl_pasang);
                    const serviceDueDate = new Date(
                      tglPasang.getTime() +
                        data.umur_service * 24 * 60 * 60 * 1000,
                    );
                    const today = new Date();

                    // Reset time to compare only dates
                    today.setHours(0, 0, 0, 0);
                    serviceDueDate.setHours(0, 0, 0, 0);

                    // Calculate days remaining
                    const timeDiff = serviceDueDate.getTime() - today.getTime();
                    const daysRemaining = Math.ceil(
                      timeDiff / (1000 * 60 * 60 * 24),
                    );

                    // Calculate 30% threshold
                    const thirtyPercentThreshold = Math.ceil(
                      data.umur_service * 0.3,
                    );

                    // Determine background color
                    let backgroundColor = '';
                    if (daysRemaining <= 0) {
                      backgroundColor = '#ffe2e2'; // Service due or overdue
                    } else if (daysRemaining <= thirtyPercentThreshold) {
                      backgroundColor = '#FDE68A'; // 30% or less days remaining
                    }
                    return (
                      <tr
                        key={index}
    ...
    try {
      await axios.post(
        url,
        {
          ...
          tgl_rusak: formValues.tgl_rusak,
          keterangan: formValues.keterangan,
          sisa_umur: formValues.sisa_umur,
        },
        {
          withCredentials: true,
        },
